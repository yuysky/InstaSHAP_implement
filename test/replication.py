...
# ########################################################################
#  EXPERIMENT 3 :  Tree Cover  (Section 6.3)
# ########################################################################
def run_treecover_experiment():
    """
    Requires the covertype dataset at ../data/covertype/covtype.data.gz
    Trains classification InstaSHAP with elevation×soil interaction.
    """
    print("\n" + "=" * 60)
    print("  EXPERIMENT 3: Tree Cover  (Section 6.3)")
    print("=" * 60)

    try:
        from dataloader import CustomTreeDataset
        dataset_obj = CustomTreeDataset(root_dir="../data/", dataset_name="covertype", seed=37)
    except Exception as e:
        print(f"Could not load tree cover data: {e}")
        print("Skipping tree cover experiment. Place covtype.data.gz in ../data/covertype/")
        return

    from instashap import InstaSHAP_classifier, surrogate
    from sian.models import TrainingArgs
    ...
    d = trnX.shape[1]
    num_classes = dataset_obj.get_C()
    print(f"Features: {d}, Classes: {num_classes}")

    # --- Step 1 & 2: Use SIAN to discover interactions ---
    results_path = "./results/"
    exp_folder = results_path + "tree_experiment/"
    os.makedirs(exp_folder, exist_ok=True)

    mlp_args = TrainingArgs(batch_size=64, number_of_epochs=10, learning_rate=5e-3, device=device)
    mlp_args.model_config.net_name = "MLP"
    mlp_args.model_config.sizes = [-1, 128, 256, 128, -1]
    mlp_args.model_config.is_masked = True
    mlp_args.saving_settings.exp_folder = exp_folder
    ...
    my_surrogate = surrogate(mlp_args=mlp_args, dataset_obj=dataset_obj,
                             max_number_of_rounds=18, order=2,
    ...
    # --- Step 3: Train InstaSHAP classifier ---
    # subsample for speed
    n_sub = min(50000, trnX.shape[0])
    rng = np.random.default_rng(42)
    idx = rng.choice(trnX.shape[0], n_sub, replace=False)
    trnX_sub = trnX[idx]
    trnY_sub = trnY[idx]

    X_tr = torch.from_numpy(trnX_sub).float()
    Y_tr = torch.from_numpy(trnY_sub.squeeze()).long()
    train_ds = TensorDataset(X_tr, Y_tr)
    train_loader = DataLoader(train_ds, batch_size=256, shuffle=True)

    print("Training InstaSHAP classifier with discovered interactions...")
    model = InstaSHAP_classifier(interactions, transform_matrix,
                                 num_classes=num_classes, device=device)
    model.train_instaSHAP(train_loader, num_epochs=30, lr=5e-3)

    # Evaluate accuracy
    model.eval()
    X_val = torch.from_numpy(valX).float().to(device)
    Y_val = torch.from_numpy(valY.squeeze()).long()
    with torch.no_grad():
        full_mask = torch.ones_like(X_val)
        logits = model(X_val, full_mask)
        preds = logits.argmax(dim=1).cpu()
        acc = (preds == Y_val).float().mean().item() * 100
    print(f"Validation Accuracy: {acc:.1f}%")

    # Plot elevation × soil partial dependence (Figure 5 style)
    print("Plotting elevation × soil interaction...")

    # Find an elevation(0) × soil_col interaction in discovered set
    elev_soil_idx = None
    elev_soil_inter = None
    for idx_i in range(1, len(interactions)):
        inter = interactions[idx_i]
        if 0 in inter and any(s in inter for s in range(14, 18)) and len(inter) == 2:
            elev_soil_idx = idx_i - 1
            elev_soil_inter = inter
            break

    if elev_soil_idx is None:
        print("elevation×soil interaction not discovered. Plotting 1D elevation dependence instead.")
        # fallback: plot the elevation singleton shape function
        elev_idx = 0  # singleton for feature 0
        phi_elev = model.model[elev_idx]
        elevations = np.linspace(trnX[:, 0].min(), trnX[:, 0].max(), 50).astype(np.float32)
        grid_t = torch.from_numpy(elevations.reshape(-1, 1)).float().to(device)
        with torch.no_grad():
            vals = phi_elev(grid_t).cpu().numpy()
        fig, ax = plt.subplots(figsize=(8, 5))
        tree_names = ["Spruce/Fir", "Lodgepole", "Ponderosa", "Cottonwood", "Aspen", "Douglas-fir", "Krummholz"]
        for c in range(min(vals.shape[1], 7)):
            ax.plot(elevations, vals[:, c], label=tree_names[c])
        ax.set_xlabel("Elevation (m)")
        ax.set_ylabel("φ_elevation per class")
        ax.set_title("Tree Cover: 1D Elevation Shape Function")
        ax.legend(fontsize=7)
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("fig5_treecover_elevation.png", dpi=150)
        plt.show()
    else:
        print(f"Found interaction: features {elev_soil_inter}")
        phi_es = model.model[elev_soil_idx]
        soil_col = [s for s in elev_soil_inter if s != 0][0]

        elevations = np.linspace(trnX[:, 0].min(), trnX[:, 0].max(), 50).astype(np.float32)
        soil_vals = np.unique(trnX[:, soil_col])[:4]  # up to 4 categories
        soil_labels = ["lower montane", "upper montane", "subalpine", "alpine"]

        fig, ax = plt.subplots(figsize=(8, 5))
        for s_idx, s_val in enumerate(soil_vals):
            grid = np.column_stack([elevations, np.full_like(elevations, s_val)])
            grid_t = torch.from_numpy(grid).float().to(device)
            with torch.no_grad():
                vals = phi_es(grid_t).cpu().numpy()
            label = soil_labels[s_idx] if s_idx < len(soil_labels) else f"soil={s_val}"
            ax.plot(elevations, vals[:, 0], label=label)
        ax.set_xlabel("Elevation (m)")
        ax.set_ylabel("φ_{elevation,soil} (class 0)")
        ax.set_title("Tree Cover: Elevation × Soil Interaction")
        ax.legend()
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("fig5_treecover_interaction.png", dpi=150)
        plt.show()
        print("Saved fig5_treecover_interaction.png")
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--exp", type=str, default="synthetic",
    #                     choices=["synthetic", "bike", "tree", "all"],
    #                     help="Which experiment to run")
    # parser.add_argument("--n_test", type=int, default=200,
    #                     help="Number of test samples for synthetic (ground-truth SHAP is slow)")
    # parser.add_argument("--epochs", type=int, default=300,
    #                     help="Training epochs for synthetic experiment")
    # args = parser.parse_args()
    # run_synthetic_experiment()
    run_bike_sharing_experiment()
    # if args.exp in ("tree", "all"):
    #     run_treecover_experiment()
# %%
